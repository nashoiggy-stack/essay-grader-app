...
interface EssayHistorySidebarProps {
  ...
  // Phase 14: the current editor text — used to decide whether restoring
  // a saved version would replace unsaved work. When non-empty and
  // different from the target entry, restore prompts a confirm step.
  readonly currentEssayText?: string;
}
...
export const EssayHistorySidebar: React.FC<EssayHistorySidebarProps> = ({
  essays, isOpen, onToggle, onLoad, onDelete, onRename, currentEssayText = "",
}) => {
  ...
  const restore = (essay: SavedEssay) => {
    const current = currentEssayText.trim();
    const target = essay.essayText.trim();
    // If the editor is empty or matches the saved version exactly, no
    // confirm needed — restore immediately.
    if (!current || current === target) {
      onLoad(essay.id);
      setConfirmRestoreId(null);
      return;
    }
    // Otherwise ask for one confirmation before overwriting unsaved text.
    if (confirmRestoreId === essay.id) {
      onLoad(essay.id);
      setConfirmRestoreId(null);
    } else {
      setConfirmRestoreId(essay.id);
    }
  };
  ...
  return (
    <>
      ...
      <motion.button
        ...
        whileTap={{ scale: 0.97 }}
        className="fixed right-4 top-20 z-40 rounded-xl bg-[#12121f] border border-white/[0.08] px-3 py-2.5 text-xs font-semibold text-zinc-400 hover:text-zinc-200 hover:border-white/[0.15] transition-[background-color,color,border-color] duration-200 shadow-black/40 flex items-center gap-2"
      >
                    ...
                    >
                      ...
                      {editingId === essay.id ? (
                        <div className="flex gap-1.5 mb-1">
                          <input
                            ...
                            onBlur={commitRename}
                            className="flex-1 bg-white/[0.05] border border-accent-line rounded-md px-2 py-0.5 text-xs text-zinc-200 focus:outline-none"
                          />
                        </div>
                      ) : (
                        <button
                      ...
                      <div className="flex items-center justify-between mt-1">
                        ...
                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          {confirmRestoreId === essay.id ? (
                            <button
                              onClick={(e) => { e.stopPropagation(); restore(essay); }}
                              className="text-amber-400 text-[10px] font-semibold px-1"
